...
plot<-ggplot(data=price_lib,aes(x=year,y=value,shape=country,group=country))+
  ...
  labs(shape="Country",x="Year",y="Price liberalisation",title = "Price Liberalisation by Year" )
plot
...
financial <- data.frame(trade$year,trade$country,banking$value,securities$value,trade$value)
financial$sum <- (financial$banking.value+financial$securities.value+financial$trade.value)
financial$average <- financial$sum/3
